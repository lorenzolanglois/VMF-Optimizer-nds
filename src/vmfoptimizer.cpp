...
void optimizeWorld(Stats *stats, ifstream &file, ofstream &out, string &line) {
    while (getline(file, line) && line != "entity") {
        if (line.find("vertices_plus") != string::npos) {
...
void optimizeEntities(Stats *stats, ifstream &file, ofstream &out, string &line) {
    ...
    while (getline(file, line)) {
        if (line.length() > 4) {
            if (line.find("classname") == string::npos &&
                line.find("vertices_plus") == string::npos) {
                switch (type) {
                    case 0:
                        out << removeExtraCharacters(line);
                        break;
                    case 1: // prop_static
                        if (line.find("angles\" \"0 0 0") == string::npos &&
                            ...
                            line.find("lightmapresolutionx\" \"32") == string::npos &&
                            line.find("lightmapresolutiony\" \"32") == string::npos &&
...
void optimizer() {
    ...
    while (1) {
        ...
        cout << "\nOptimizing " << userInput << " . . .\n";
        optimizeWorld(&stats, file, out, line);
        optimizeEntities(&stats, file, out, line);
